...
import { SCHEMA_skill_update } from "@/app/lib/schemas";
import { useRouter } from "next/navigation";
import Spanning from "@/app/components/spanning";
import Image from "next/image";
import { I_categories_data, I_skill } from "@/app/lib/types/categories";
import { updateSkillAction } from "@/app/lib/actions/categories";
...
function AdminUpdateSkill({ setClose, token, skills, data_categories }: Props) {
  ...
  const [state, formAction, isPending] = useActionState(
    async (prevState: any, form: FormData) => {
      ...
      if (image) {
        result = SCHEMA_skill_update.safeParse({
          name: formData.name,
        ...
        const TextSchema = SCHEMA_skill_update.omit({ avatar: true });
      ...
      if (!isUpdate) {
        toast.warning("Bạn chưa thay đổi thông tin kỹ năng!");
        return { success: false };
      }

      update_data.append("token", token);
      update_data.append("skillId", skills.id);
      ...
      // ✅ Call Server Action
      const actionResult = await updateSkillAction(prevState, update_data);
      ...
      <form action={formAction} className="mt-5">
        ...
        <div className="mt-6">
          ...
          <div className="flex justify-end gap-2 items-center">
            ...
            >
              {isPending ? <Spanning /> : "Cập nhật kỹ năng"}
            </button>
          </div>
        </div>
      </form>
