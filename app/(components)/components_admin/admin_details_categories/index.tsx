...
import AdminCreateSkill from "@/app/(components)/components_admin/admin_create_skill";
import AdminUpdateSkill from "@/app/(components)/components_admin/admin_update_skill";
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(
        `/apiFe/skills/search?categoryId=${category_id}&keyword=${searchQuery}&page=1&per_page=3`
      );
      if (!response.ok) {
  ...
  const handlePageChange = async (page: number) => {
    ...
    try {
      const url = searchQuery
        ...
        : `/apiFe/categories/${category_id}/skills?page=${page}&per_page=3`;
